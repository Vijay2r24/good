...
function AddOrders() {
  ...
  const handleCustomerSelect = (customer) => {
    ...
    console.log("vijakumar", selectedCustomer); // Set selected customer
  ...
  const handleAutoFill = (selectedAddressID) => {
    console.log("vijaykumar", selectedAddressID);
  
    // Check if order.Customer exists before trying to access its properties
    if (!order.Customer) {
      console.error("Customer information is missing in the order");
      return;
    }
  
    // Find the address that matches the selected AddressID
    const selectedAddress = order.Customer?.Address?.find(
    ...
    console.log("tuesday", selectedAddress.AddressLine1);
    ...
    setOrderDetails((prevDetails) => ({
      ...
      CustomerFirstName:
        selectedCustomer.CustomerFirstName || prevDetails.CustomerFirstName || "",
      CustomerLastName:
  ...
  useEffect(() => {
    ...
    if (isEditMode) {
      ...
      console.log("vijaykumarnaidu", order.SubStatusId)
      setStatusUpdatedData(orderDetails.OrderStatus);
      setUpdatedSubStatusId(order.SubStatusId);
      console.log("vijaykumarnaidu", updatedsubStatusId)
                            ...
                            <div className="pt-4">
                              ...
                              >
                                <Table stickyHeader>
                                  ...
                                  <TableBody>
                                    {orders?.map((order) =>
                                      order.Customer?.Address?.map((address, index) => (
                                        <TableRow key={index}>
                                          ...
                                          <StyledTableCell align="center">
                                            <div className="button-container">
                                              <button
                                                type="button"
                                                onClick={() => {
                                                  handleAutoFill(address.AddressID);  // Pass AddressID to autofill
